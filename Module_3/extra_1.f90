program questions
    ...
    !L = 12 for first neighbouring spin -------------------------------------------
    len = 12

    open(unit=1, file="E:\computational_physics\Module_3_out\extra_1_data.dat")

    do i = 600, 300, -2
        call ising_model(len, 1.0, i/100.0, 10**5, "random", answers, 1, "dont write")
        write(1,*) i/100.0, ",", answers(1)/(len**3), ",", answers(2)/(len**3), ",", &
        answers(3), ",", answers(4), ",", answers(5)/(len**3), ",", answers(6), ",", answers(7)

        print *, "temperature =", i/100.0, "model = 1"
    end do
    
    close(1)

    !L = 13 for first neighbouring spin -------------------------------------------
    len = 13

    open(unit=2, file="E:\computational_physics\Module_3_out\extra_2_data.dat")

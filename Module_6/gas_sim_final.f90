module gas_sim_final
    implicit none 
    
    contains


    subroutine gas_simulation_final(num_particles, mass, r_cutoff, epsilon, sigma, temperature, dt, run_time, &
               sample_iter, len, output_values, thermostat, corr_path, neigh_path, speed_path)
        ...
        character(len=*) :: thermostat, corr_path, neigh_path, speed_path
        real*8 :: r_nearby 
        integer, allocatable :: num_neighbour(:), neighbour_list(:,:)
        real*8 :: dr, dv, r_max
        real*8, allocatable :: correlation_list(:,:), speed_list(:,:)
        integer :: num_bins, v_bins 
...
!!!================================= loops strart ===================================!!!
        ...
        do i = 1, run_time
            ...
            if (i > 20000 .and. mod(i,100) == 0) then
                ...
                call speed_distro(num_particles, v_list, v_bins, speed_list)
            end if 
        ...
        do i = 1, v_bins
            speed_list(2,i) = speed_list(2,i)/count_2
        end do

        open(file = speed_path, unit = 3)
        do i = 1, size(speed_list(1,:))
            write(3,*) speed_list(1,i), speed_list(2,i), speed_list(3,i)
        end do
        close(3)

    end subroutine gas_simulation_final
    ...
    subroutine speed_distro(num_particles, v_list, v_bins, speed_list)
        ...
        integer :: bin, v_bins
        real*8 ::  dv, pi
        real*8 :: speed_dist(v_bins), v_list(3*num_particles), speed_list(3,v_bins)
        integer :: v_hist(v_bins)
        ...
        do i = 1, num_particles
            speed = dsqrt(v_list(3*i)**2 + v_list(3*i-1)**2 + v_list(3*i-2)**2)
            
            if (speed < 15.0d0) then
                bin = int(speed/dv) + 1
                if (bin <= v_bins) v_hist(bin) = v_hist(bin) + 1
            end if
        end do

        do i = 1, v_bins
            speed = (i - 0.5d0)*dv
            speed_dist(i) = dfloat(v_hist(i))/(num_particles*dv)

            speed_list(1,i) = speed
            speed_list(2,i) = speed_list(2,i) + speed_dist(i)
            speed_list(3,i) = dsqrt(2.0d0/pi)*(speed**2)*exp(-0.5d0*speed**2)
        end do

    end subroutine speed_distro


end module gas_sim_final